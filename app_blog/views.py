...
class SeriesUpdateView(UpdateView):
    ...
    success_url = reverse_lazy(
        "app_blog:series-list"
    )  # when a Post is successfully updated, redirect to this url
...
def replyComment(request, pk):
    ...
    if request.method == "POST":
        ...
        newReply = Comment(
            commenter=replier_name,
            comment_detail=reply_content,
            post=post,
...
class ReplyCommentCreateView(LoginRequiredMixin, GetURLPKMixin, CreateView):
    model = Comment
    form_class = ReplyCommentForm
    template_name = "app_blog/comment/reply_comment_form.html"
    success_url = reverse_lazy("app_blog:comment-list")

    # Ensure that a comment with the url pk exists when a user visits the page
